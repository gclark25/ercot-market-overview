...
4. How yesterday's forecast held up against what actually happened. Specifically check whether the miss was spread evenly across the day or concentrated in one time-of-day block (overnight HE1-6, morning HE7-12, afternoon HE13-18, evening HE19-24, using by_block below) — a block-specific miss is a more useful, more accurate thing to report than a flat day-level average that may be hiding it.
...
6. Only if something below is a genuine outlier: one line flagging it. Omit this section entirely on an ordinary day — most days should NOT have this section."""]
    ...
    if highlights.get("bid_close_accuracy"):
        sections.append("Pre-computed: yesterday's actual net load vs. the forecast as of bid-close (09:00 CT the day before) — all figures are in GW (matching the net load chart's own units). mean_error_gw/mean_abs_error_gw are the flat day-level figures (signed / magnitude). by_block breaks the same signed and absolute error into four 6-hour blocks (HE1-6 overnight, HE7-12 morning, HE13-18 afternoon, HE19-24 evening) — check this before describing the miss as small or large, since a block-concentrated miss can look tiny in the day-level average:\n" + json.dumps(highlights["bid_close_accuracy"]))
